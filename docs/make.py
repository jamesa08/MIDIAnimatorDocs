...
for filename in os.listdir('docs/source/api/'):
    if ".libs." in filename\
    or "MIDIAnimator.rst" == filename\
    or "modules.rst" == filename:
        os.remove(os.path.join('docs/source/api/', filename))
...
for filename in os.listdir('docs/source/api/'):
    ...
    or "modules.rst" == filename:
        continue
    with open(os.path.join('docs/source/api/', filename), 'r') as f:
        lines = f.readlines()

    contains = False
    title = ""
    with open(os.path.join('docs/source/api/', filename), 'w') as f:
