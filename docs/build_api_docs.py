...
# os.system('rm -rf .git*')
# print(os.environ)
# os.system('git clone --recursive https://github.com/jamesa08/MIDIAnimatorDocs.git .')
...
if __name__ == '__main__':
    ...
    for filename in os.listdir('docs/api/'):
        if ".libs." in filename\
        or "MIDIAnimator.rst" == filename\
        or "modules.rst" == filename:
            os.remove(os.path.join('docs/api/', filename))
    ...
    for filename in os.listdir('docs/api/'):
        ...
        or "modules.rst" == filename:
            continue
        with open(os.path.join('docs/api/', filename), 'r') as f:
            lines = f.readlines()
